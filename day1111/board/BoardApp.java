 ...
 * 각 페이지들마다 서로 공유할 데이터가 있다면, BoardApp*/
...
public class BoardApp extends JFrame{
	...
	public BoardApp() {
		...
		//가입버튼과 리스너 연결
		//Lambda 표현식 : 함수형 프로그램 표현식 java 8 부터 지원
	...
	//로그인 여부를 판단해서 로그인하지 않았다면, 로그인 페이지 보여주기
	...
	public void getConnection() {
		try {
			...
			con = DriverManager.getConnection(url, user, pass); //접속 시도 후, 객체 반환
			if (con == null) {
	...
	public void disConnection() {
		//null이 아닐때만 닫아야함, 만일 이런 확인절차를 거치지 않으면 NullPointerException 발생할 수 있음
		if (con != null) {
			try {
				con.close();
			} catch (SQLException e) {
