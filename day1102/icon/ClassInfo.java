...
public class ClassInfo {
	...
	public ClassInfo() {
		...
		//Class 클래스를 이용하며, 해당 클래스의 환경정보등도 접근할 수 있다.. 따라서 우리는
		//클래스 패스를 조사해서 우리가 사용중인 res 라는 패키지의 경로를 조사할 것임...
		//아래의 메서드를 이용하면, 클래스 패스를 기준으로 자원에 접근할 수 있다.
		//패키지에 넣은 자원이 클래스가 아닌 일반파일인 경우 .이 아닌 /슬래쉬로 접근해야 함
//		URL url = myClass.getClassLoader().getResourceAsStream("res/instagram.png");
