...
public class MemoApp extends JFrame{
	...
	public MemoApp() {
		...
		for (int i = 0; i < item_title.length; i++) {
			...
			m_file.add(items[i]); //파일 메누에 아이템 부착
		}
		...
		//프레임에 scroll 부착(얼핏 보기엔 area를 부착해야 할 것처럼 보이지만, scroll 이 area를 포함하고 있으므로
		//scroll을 붙여야 한다.)
		add(scroll);
		...
		area.setFont(new Font("궁서체", Font.BOLD|Font.ITALIC, 20)); //Font 객체를 사용하는 법
