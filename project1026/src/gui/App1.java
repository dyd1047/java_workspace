...
ex) 버튼, 텍스트 필드, 라디오박스, 체크박스, 초이스, 이미지...
*/
package gui;
...
class App1{
	public static void main(String[] args){
		...
		//2. 추상클래스 : new 불가하므로, 자식을 정의해서 new 하거나 이미 존재하는 인스턴스를 이용하면 된다.(api문서를 통해)
		...
		frame.setVisible(true);//Window 객체로부터 상속받는 메서드
		...
		//버튼을 부착하기 전에, 레이아웃 스타일을 지정해야 하는데, 레이아웃은 추후 배울거니까
		//일단 FlowLayout을 사용해보자
		FlowLayout flow = new FlowLayout();
		frame.setLayout(flow);//윈도우에 플로우방식의 배치적용
		...
		//html 에서의 imput type = "text"는 자바에서는 TextField라 한다.
		...
		//지금 이 자바코드가 모든 os에서 실행되려면, 사실 중립적인 경로로 가야한다.
		...
		//지금까지 html에서는 이미지를 문서에 덧붙여서 구현했으나, 자바와 같은 일반적인 컴파일기반의 프로그래밍 언어에서는 랜더링(직접 그리는 작업)을 해야하기 때문이다.
