...
public class RegistForm extends JPanel{
	...
	public RegistForm(Product product) {
		...
		bt_regist = new JButton("Add");
		bt_list = new JButton("List");
		
		//최상위 카테고리 채우기(DB연동 X, 기존 데이터로 재사용)
		...
		bt_regist.addActionListener((e)->{
			if(regist() == 0) {
				...
				JOptionPane.showMessageDialog(RegistForm.this, "등록 성공");
	...
	//유저가 선택한 아이템으로부터 sucategory의 pk를 가져오기
	...
	public int regist() {
		...
		try {
			...
			pstmt.setString(6, t_detail.getText()); //상세보기
