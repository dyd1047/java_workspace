/*이 클래스는 페이지 수준은 아니고, 단지 장바구니 페이지에서 재사용성이 높은
 * 상품 아이템을 정의하기 위한 클래스*/
package com.swingmall.cart;

import java.awt.Color;
import java.awt.Dimension;
import java.awt.Graphics;
import java.awt.GridLayout;
import java.awt.Image;

import javax.swing.JButton;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JTextField;
import javax.swing.border.LineBorder;

import com.swingmall.main.ShopMain;

import common.image.ImageUtil;

public class CartItem extends JPanel{
	JPanel p_border;
	JPanel p_can; //상품 이미지
	JPanel p_info; //라벨들이 위치할 그리드 패널(3,1)
	JLabel la_brand, la_product_name, la_price;
	JLabel la_tBrand, la_tProduct_name, la_tPrice;
	JLabel la_ea; //수량
	JTextField t_ea;
	JButton bt_update; //수량변경 적용 버튼
	JButton bt_del;
	Image image;
	
	public CartItem(CartVO cartVO) {
		image = ImageUtil.getCustomSize(ImageUtil.getImageFromURL(cartVO.getFilename()), 100, 85);
		
		p_border = new JPanel();
		p_can = new JPanel() {
			@Override
			public void paint(Graphics g) {
				g.drawImage(image, 0, 0, p_can);
			}
		};
		p_info = new JPanel();
		la_brand = new JLabel(cartVO.getBrand());
		la_product_name = new JLabel(cartVO.getProduct_name());
		la_price = new JLabel(Integer.toString(cartVO.getPrice()));
		la_tBrand = new JLabel("Brand");
		la_tProduct_name = new JLabel("Name");
		la_tPrice = new JLabel("Price");
		la_ea = new JLabel("EA");
		t_ea = new JTextField(Integer.toString(cartVO.getEa()));
		bt_update = new JButton("Apply");
		bt_del = new JButton("Del");
		
		//스타일
		this.setPreferredSize(new Dimension(ShopMain.WIDTH-420, 150));
		this.setBackground(Color.WHITE);
		p_border.setPreferredSize(new Dimension(ShopMain.WIDTH-450, 120));
		p_border.setBackground(Color.WHITE);
		p_border.setBorder(new LineBorder(Color.BLACK, 1));
		
		p_can.setPreferredSize(new Dimension(100, 85));
		p_info.setPreferredSize(new Dimension(200, 100));
		p_info.setBackground(Color.WHITE);
		
		la_ea.setPreferredSize(new Dimension(240, 25));
		la_ea.setHorizontalAlignment(JLabel.RIGHT);
		t_ea.setPreferredSize(new Dimension(50, 25));
		bt_update.setBackground(Color.BLACK);
		bt_update.setForeground(Color.WHITE);
		bt_update.setFocusPainted(false);
		bt_del.setBackground(Color.BLACK);
		bt_del.setForeground(Color.WHITE);
		bt_del.setFocusPainted(false);
		
		//조립
		p_info.setLayout(new GridLayout(3,2));
		p_info.add(la_tBrand);
		p_info.add(la_brand);
		p_info.add(la_tProduct_name);
		p_info.add(la_product_name);
		p_info.add(la_tPrice);
		p_info.add(la_price);
		p_border.add(p_can);
		p_border.add(p_info);
		p_border.add(la_ea);
		p_border.add(t_ea);
		p_border.add(bt_update);
		p_border.add(bt_del);
		this.add(p_border);
		
		p_can.repaint();
	}
}
