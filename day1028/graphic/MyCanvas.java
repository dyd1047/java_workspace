...
public class MyCanvas extends Canvas{
	Toolkit kit; //플랫폼에 따라 알맞게 자원을 가져올 수 있도록 해주는 객체
	Image img; //얘도 추상임.. 근데 툴킷을 통해 인스턴스를 얻을 수 있음..

	public MyCanvas() {
		kit = Toolkit.getDefaultToolkit(); //자체 클래스 메서드(static)를 통해 인스턴스를 얻는 방식
		img = kit.getImage("D:/User/yong/LN/workspace/athome/workspace-HTML/java_workspace/SeProject/res/mario/mario.png"); //윈도우 os
	}
	...
	public void paint(Graphics g) {
		...
		//			  일반, 추상, 인터페이스
		//조사해보니, 이미지는 추상클래스!!
		//추상클래스인 경우, 개발자가 자식클래스로 구현 후, 자식을 new 해서 사용해야 함이 원칙이지만,
		...
		//Toolkit 객체를 이용해야 하므로, 이미지를 얻기전에 우선 툴킷 객체의 인스턴스부터 얻자
