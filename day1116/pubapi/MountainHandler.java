...
public class MountainHandler extends DefaultHandler{
	...
	public void startElement(String uri, String localName, String tag, Attributes attributes) throws SAXException {
		System.out.println("<"+tag+">"); //여는 태그 출력
		...
		if(tag.equals("items")) { //여기서 벡터 만들고
			...
			isMntnid = true;
		}else if(tag.equals("mntnnm")) { //실행부가 이름을 지나간다고 표시
			isMntnnm = true;
		}else if(tag.equals("mntninfopoflc")) { //실행부가 소재지를 지나간다고 표시
			isMntninfopoflc = true;
		}else if(tag.equals("mntninfohght")) { //실행부가 높이를 지나간다고 표시
			isMntninfohght = true;
		}
	}
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		if(isMntnid) {
			...
			mt.setMntninfopoflc(data); //산의 소재지 담기
...
//		System.out.println("</"+tag+">"); //닫는 태그 출력
		
		//데이터를 담은 후 실행부의 위치를 초기화
