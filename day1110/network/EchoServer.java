...
public class EchoServer {
	...
	ServerSocket server; //대화용 소켓이 아니라, 접속자를 감지하고, 접속자가 발견되면 대화용 소켓을 반환해주는 객체(마치 전화기의 벨이 울리면
								//그 이후부터 전화를 받고 대화하는 것과 비슷) 전화 받는용 소켓
	
	public EchoServer() {
		try {
			server = new ServerSocket(port); //서버소켓 생성(아직 가동은 아님)
			Socket socket = server.accept(); //서버가동 및 클라이언트 접속 기다림...(접속이 들어올때까지 무한 대기 한다. 지영발생)
			...
			String data = null;
			while(true) {
				data = buffr.readLine(); //1byte 읽기 (소켓과 연결된 스트림으로 부터)
