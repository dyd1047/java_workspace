...
public class EchoServer extends JFrame{
	...
	public EchoServer() {
		...
		setVisible(true);
		setBounds(600, 200, 350, 450); //x,y,width,height 순선
	}
	...
	public void startServer() {
		try {
			...
			//서버 가동하기
			//자바는 쓰레드 기반이므로, 지금까지 메인 실행부라 불렸던 실행주체도 사실은 시스템에 의해 생성된 쓰레드였다.
			//하지만, 메인쓰레드는 개발자가 생성하는 일반 쓰레드와는 하는 역할에 차이가 있다.
			//메인쓰레드는 프로그램을 운영해주는 역할 특히 그래픽처리, 이벤트처리까지 담당하므로, 절대로 아래의 업무금지
			//1. 무한루프에 빠뜨리지 말것
			//2. 대기상태에 빠지게 하지 말것 (accept(), 스트림의 read()... 등등)
			//	참고로 안드로이드에서는 메인 쓰레드의 1, 2번 시도자체를 에러로 본다.
	...
	public void listen() {
		...
		try {
			while(true) {
				msg = buffr.readLine(); //현재로서는 한 번만 듣는다.
				area.append(msg+"\n");
