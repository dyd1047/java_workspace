...
/*
 *"5" --> 정수 5로 바꾸려면? js : pareInt()
 *
 * 자바에서는 모든 기본 자료형 마다 1:1 대응하는 래퍼클래스를 지원해준다.
...
public class WrapperApp {
	public static void main(String[] args) {
		...
		Integer i = 5; //자바의 클래스 원칙으로 본다면 저런 식은 원래 불가능하다.. 하지만 Integer 클래스는
							//기본 자료형과 관련된 객체이므로, 마치 기본자료형처럼 데이터를 대입할 수 있다.
							//사실상 내부적으로 5라는 기본 자료형이 객체화 된것이다..(Boxing : 기본 데이터를 박스로 감쌌다.)
							//Wrapper 클래스는 box로 감싸다(Wrapper)에서 근거한 말임
		int k = i; //원칙상 객체자료형을 기본자료형에 넣으려고 했으니, 불가능한 일이지만 위와 마찬가지로 래퍼클래스는
					//기본 자료형과 관련된 객체이므로, 내부적으로 unBoxing에 의해 기본 자료형으로 변환이 된것이다.
		//결론 : 기본 자료형을 --> 객체화 (Boxing), 객체자료형을 --> 기본자료형으로 (unBoxing)
